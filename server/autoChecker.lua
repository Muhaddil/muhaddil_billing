        ...
        if statusCode == 200 then
            ...
            if data and data.tag_name then
                ...
                if latestVersion ~= currentVersion then
                    ...
                    print('╭────────────────────────────────────────────────────╮')
                    printWrapped('[muhaddil_billing] - Up-to-date', boxWidth, '32')     -- Green
                    printWrapped('Current version: ' .. currentVersion, boxWidth, '32') -- Green
                    print('╰────────────────────────────────────────────────────╯')
                end
            else
                printWithColor('[muhaddil_billing] - Error: The JSON structure is not as expected.', '31') -- Red
                printWithColor('GitHub API Response: ' .. response, '31')                                  -- Red
            end
        else
            printWithColor(
                '[muhaddil_billing] - Failed to check for latest version. Status code: ' .. statusCode, '31') -- Red
        end
    end, 'GET')
end
